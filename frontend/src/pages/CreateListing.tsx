...
const CreateListing: React.FC = () => {
  ...
  // Ładowanie podkategorii po wyborze kategorii
  useEffect(() => {
    const loadSubcategories = async () => {
      ...
      const res = await fetch(
        `${API_BASE}/api/listings/subcategories?category_id=${categoryId}`,
        {
          headers: { ...(API_KEY ? { 'x-api-key': API_KEY } : {}) },
        }
      );
  ...
  const handleFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of selected) {
      ...
      if (!ALLOWED_TYPES.includes(file.type)) {
        newErrors.push(
          `Niedozwolony typ pliku: ${file.name}. Dozwolone: JPG/PNG/WebP.`
        );
        continue;
      }
      ...
      if (sizeMB > MAX_FILE_SIZE_MB) {
        newErrors.push(
          `Plik ${file.name} jest za duży (${sizeMB.toFixed(
            1
          )} MB). Maksymalnie ${MAX_FILE_SIZE_MB} MB.`
        );
        continue;
      }
      current.push(file);
  ...
  const moveImage = (idx: number, direction: 'left' | 'right') => {
    ...
    [newImages[idx], newImages[newIndex]] = [
      newImages[newIndex],
      newImages[idx],
    ];
    [newPreviews[idx], newPreviews[newIndex]] = [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!type) {
      alert('Wybierz typ ogłoszenia');
      return;
    }
    if (!categoryId) {
      alert('Wystąpił błąd: brak kategorii dla wybranego typu');
      return;
    }
    if (!subcategoryId) {
    ...
    // Konwersja zdjęć tylko dla sprzedaży
    ...
    if (type === 'work') {
      if (!salary) {
        alert('Podaj wynagrodzenie');
        return;
      }
      if (!jobMode) {
        alert('Wybierz tryb pracy');
        return;
      }
      if (!jobCategory) {
        alert('Podaj kategorię stanowiska');
        return;
      }
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Błąd dodawania ogłoszenia:', data);
        alert(
          'Błąd: ' +
            (data?.error || data?.message || 'nie udało się dodać ogłoszenia')
        );
        return;
      }
              ...
              <div className="images-header">
                <div className="images-text">
                  ...
                  <div className="images-subtitle">
                    (JPG/PNG/WebP, do {MAX_FILES} plików, max {MAX_FILE_SIZE_MB}{' '}
                    MB każdy)
                  </div>
                </div>
